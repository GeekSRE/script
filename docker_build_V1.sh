...
Version=`ls $Package_Dir/$NewPackage|awk -F'_' '{print $4}'|awk -F'.' '{print $1}'`
...
cd $Work_Dir
docker build -t $Harbor_Url/project/$1:$Version .

docker login $Harbor_Url -u $Harbor_U -p $Harbor_P

docker push dockreg.biyao.com/project/$1:$Version

[ $? -eq 0 ] && echo 'docker push ok ! '

rm -f $Work_Dir/$NewPackage
mv Dockerfile Dockerfile-$1:$Version